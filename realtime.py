...
img = cv2.imread("python_test\seam-carving-master\imgs/01.jpg", cv2.IMREAD_COLOR)
# cv2.imshow("1",img)
# print(img)
img_masked = img.copy()
mask = np.zeros(img.shape[:2], np.uint8)

sketcher = Sketcher('image', [img_masked, mask],
...
while True:
    ...
    if key == 32:  # hit spacebar
        new_width = int(cv2.getTrackbarPos(
            trackbarname='width', winname='image'))
        new_height = int(cv2.getTrackbarPos(
            trackbarname='height', winname='image'))
        ...
        if np.sum(mask) > 0:  # object removal or protect mask
            if MODE == 'remove':
                ...
                carver = SeamCarver(
                    img, new_height, new_width, protect_mask=mask)
            else:
